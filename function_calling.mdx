...
description: "Learn how to set up your server to handle function calls from Vapi."
---
...
This is for if you need to customize the agent's context by retrieving it from your server. If `callbackUrl` is specified and `context` is not, the agent will make a `GET` request to the `callbackUrl` with the `customerPhoneNumber` and `agentId` as query parameters. Your server should respond with a JSON object containing the context, like so:
