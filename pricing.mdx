---
title: "Pricing"
sidebarTitle: "Pricing"
description: "Get to know your pricing per minute based on the providers you choose."
---

The vast majority of the cost of using Vapi is the cost of the underlying providers. We charge a base fee on top of the provider cost to cover our costs and to be a sustainable business.

When using [Provider Keys](/provider_keys), you'll be billed directly by the provider, not us. When not using Provider Keys, you'll be billed at at cost for those providers through us.

Based on the providers you select and the keys you provide, you will be automatically billed accordingly. Below are the estimated costs per minute for each provider:

<Card title="Base Cost">
  Vapi: **\$0.05/min** (=\$3.00/hr)
</Card>
<Card title="Transcription Provider">
  Deepgram: **\$0.01/min** (=\$0.60/hr)
</Card>
<Card title="Model Provider">
  - OpenAI (gpt-4-turbo): **\$0.20/min** (=\$12.00/hr) <br /> OpenAI (gpt-3.5-turbo):
  **\$0.02/min** (=\$1.20/hr)
</Card>
<Card title="Voice Provider">
  - ElevenLabs: **\$0.10/min** (=\$6.00/hr) <br /> PlayHT: **\$0.10/min** (=\$6.00/hr)
  <br /> Deepgram: **\$0.05/min** (=\$3.00/hr) <br /> OpenAI: **\$0.02/min** (=\$1.20/hr) <br /> RimeAI: **\$0.02/min** (=\$1.20/hr)
</Card>

These costs are subject to change as we get more data, but they will always reflect our best estimate of our per minute. You can find your usage and billing information in the [dashboard](https://dashboard.vapi.ai).
